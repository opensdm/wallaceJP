...
shinyServer(function(input, output, session) {
  ...
  observeEvent(logInit(), {
    intro <- '***WELCOME TO WALLACE***'
    expl <- "Please find messages for the user in this log window."
    rvs$logs <- gsub("\\*\\*\\*WELCOME TO WALLACE\\*\\*\\*",i18n$t(intro),rvs$logs)
    rvs$logs <- gsub(expl,i18n$t(expl),rvs$logs)
  ...
  observeEvent(input$goResetOccs, {
    ...
    # reset for RMD
    rvs$comp2 <- NULL
    rvs %>% writeLog(i18n$t("Reset occurrences."))
    map %>%
      clearMarkers() %>%
      map_plotLocs(rvs$occs) %>%
      zoom2Occs(rvs$occs)
  ...
  output$ctrLatLon <- renderText({
    paste(i18n$t('Using map center'), paste(mapCntr(), collapse=', '))
  })
  ...
  observeEvent(input$goMaxent, {
    ...
    output$evalTbls <- renderUI({
      tabsetPanel(
        tabPanel("Evaluation", 
                 tagList(
                   br(),
                   div(i18n$t("Full model and partition bin average evaluation statistics"), id="stepText"), br(), br(),
                   DT::dataTableOutput('evalTbl'), br(), 
                   div(i18n$t("Individual partition bin evaluation statistics"), id="stepText"), br(), br(),
                   DT::dataTableOutput('evalTblBins')  
                 )),
        tabPanel("Lambdas",
                 br(),
                 div(i18n$t("Maxent Lambdas File"), id="stepText"), br(), br(),
    ...
    updateRadioButtons(session, "visSel", 
                       ...
                       #               "Map Prediction" = 'map'))
                       choiceNames = list(i18n$t("Maxent Evaluation Plots"), i18n$t("Plot Response Curves"), i18n$t("Map Prediction")),
                       choiceValues = list('mxEval', 'response', 'map'))
  })
  ...
  observeEvent(input$goBioclim, {
    ...
    output$evalTbls <- renderUI({
      tagList(
        br(),
        div(i18n$t("Full model and partition bin average evaluation statistics"), 
            id = "stepText"), br(), br(),
        DT::dataTableOutput('evalTbl'), br(), 
        div(i18n$t("Individual partition bin evaluation statistics"), 
            id = "stepText"), br(), br(),
        DT::dataTableOutput('evalTblBins') 
      )
    })
    ...
    updateRadioButtons(session, "visSel", 
                       ...
                       #               "Map Prediction" = 'map'))
                       choiceNames = list(i18n$t("BIOCLIM Envelope Plots"), i18n$t("Map Prediction")),
                       choiceValues = list('bcPlots', 'map'))               
    shinyjs::show(id = "evalTblBins")
  })
  ...
  output$modSelUI <- renderUI({
    ...
    selectInput('modSel', label = i18n$t("Current Model"),
  ...
  output$envSelUI <- renderUI({
    ...
    selectInput("envSel", i18n$t("Current Env Variable"),
  ...
  observeEvent(input$goResetProj, {
    ...
    rvs %>% writeLog(i18n$t("Reset projection extent."))
  })
  ...
  output$dlRMD <- downloadHandler(
    ...
    content = function(file) {
      ...
      owd <- setwd(tempdir())
      on.exit(setwd(owd))
      writeLines(exp, 'userReport2.Rmd',useBytes=T)
      
      if (input$rmdFileType == 'Rmd') {
        out <- rmarkdown::render('userReport2.Rmd', rmarkdown::md_document(variant="markdown_github"),encoding = "UTF-8")
        writeLines(gsub('``` r', '```{r}', readLines(out,encoding = "UTF-8")), 'userReport3.Rmd',useBytes=T)
        out <- 'userReport3.Rmd'
      } else {
