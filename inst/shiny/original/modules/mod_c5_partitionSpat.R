...
partSp_MOD <- function(input, output, session, rvs) {
  ...
  reactive({
    if (is.null(rvs$bgMsk)) {
      rvs %>% writeLog(type = 'error', "Before partitioning occurrences, mask your environmental variables by your background extent.")
      return()
    ...
    if (input$partSpSel == 'cb1' | input$partSpSel == 'cb2') {
      if (is.na(input$aggFact) | input$aggFact <= 1) {
        rvs %>% writeLog(type = 'error', "Please specify a positive aggregation factor greater than 1.")
        return()
      }
    }
