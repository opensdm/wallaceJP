...
queryDb_UI <- function(id) {
  ns <- NS(id)
  tagList(
    radioButtons(ns("occDb"), i18n$t("Choose Database"),
                 choices = list("GBIF" = 'gbif',
                                "VertNet" = 'vertnet',
                                "BISON" = 'bison'), inline = TRUE),
    tags$div(title=i18n$t("Examples: Felis catus, Canis lupus, Nyctereutes procyonoides"),
             textInput(ns("spName"), label = i18n$t("Enter species scientific name"), placeholder = 'format: Genus species')),
    tags$div(title=i18n$t("Maximum number of occurrences recovered from databases. Downloaded records are not sorted randomly: rows are always consistent between downloads."),
             numericInput(ns("occNum"), i18n$t("Set maximum number of occurrences"), value = 100, min = 1))
  )
}
...
queryDb_MOD <- function(input, output, session, rvs) {
  ...
  query <- reactive({
    ...
    if (nameSplit != 2) {
      rvs %>% writeLog(type = 'error', i18n$t("Please input both genus and species names."))
      return()
    }
    
    # query database
    withProgress(message = paste(i18n$t("Querying"), input$occDb, "..."), {
    ...
    if (q[[input$occDb]]$meta$found == 0) {
      rvs %>% writeLog(type = 'error', i18n$t('No records found for '), 
                     spName(), i18n$t(". Please check the spelling."))
  ...
  dbOccs.out <- reactive({
    ...
    rvs %>% writeLog(i18n$t('Total'), input$occDb, i18n$t('records for'), spName(), i18n$t('returned ['), nrow(dbOccs.tbl()),
                   i18n$t('] out of ['), totRows, i18n$t('] total (limit '), input$occNum, i18n$t('). Records without coordinates removed ['), noCoordsRem, i18n$t(']. Duplicated records removed ['), dupsRem, i18n$t(']. Remaining records ['), nrow(recs), i18n$t('].'))
