...
このモジュールでは、Rパッケージの`predicts`の多次元の類似度面を計算する`mess()`関数を使用します(Elith et al. 2010)。

ユーザーはまずモデルを選択します。手順6: **モデル化**で選択した`ENMeval`の設定によっては、Maxentの選択肢が複数になる場合があります。次に、地図上に多角形を描いてMESS計算を行う区域の輪郭を決めます。多角形を描いたら、Selectでこの範囲を選択し、Calculateで選択した範囲についてMESS分析を実行します。MESS地図は分析に使用するラスタタイプ(.tif)か画像ファイル(.png)のどちらかの形式でダウンロードできます。
