...
このモジュールではRパッケージの`ENMeval`と`predicts`を使用し、フィーチャクラスおよび正則化乗数に広範囲なモデル設定を適用してMaxentニッチ／分布モデルを構築、評価します(Muscarella et al. 2014)。
...
*注意* ：しかしながら、maxent.jarオプションでは、モデル予測は現在、トレーニングデータセットより極端な環境値のあるグリッドセル(背景サンプル＋在データレコード)について常に「クランプ」されるようになっています。これは、`predicts`における`predict()`関数のバグによるもので、モデル予測では「クランプする」オプションが常にオンになっています(すなわち、`predicts`を使ってクランプをオフにしてトレーニングデータセットの環境条件を超える無制約の外挿をすることはできません)。
