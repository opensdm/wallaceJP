...
このモジュールでは、モデル予測グリッドの機能の操作にRパッケージの`predicts`を使用します。
...
次に、ユーザーは連続予測をバイナリ予測(0と1)に変換するための閾値設定ルールを選択することができますが、これは存在／不在または好適度／不好適度を示すと解釈できます。閾値設定ルールについて詳しくは、**手順6: モデル化**の手引書を参照してください。モデルと閾値を選択した後は、地図上に予測をプロットすることができ、予測値は分析に使用するラスタタイプ(.tif)か画像ファイル(.png)のどちらかの形式でダウンロードできます。
