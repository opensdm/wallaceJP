...
projectTime_MOD <- function(input, output, session, rvs) {
  ...
  reactive({
    if (is.null(rvs$predCur)) {
      rvs %>% writeLog(type = 'error', 'Calculate a model prediction in component 7 before projecting.')
      return()
    ...
    if (!i) {
      rvs %>% writeLog(type = 'error', 'This combination of GCM and RCP is not available. Please make a different selection.')
      return()
    }
