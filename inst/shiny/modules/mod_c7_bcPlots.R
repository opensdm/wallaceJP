...
bcPlots_UI <- function(id) {
  ns <- NS(id)
  tagList(
    i18n$t("Pick a bioclimatic variable number for each axis"),
    numericInput(ns("bc1"), "Axis 1", value = 1, min = 1, max = 19),
    numericInput(ns("bc2"), "Axis 2", value = 2, min = 1, max = 19),
    numericInput(ns("bcProb"), i18n$t("Set threshold"), value = 0.9, min = 0.75, max = 1, step = 0.05),
    HTML('<hr>'),
    strong(i18n$t("Download envelope plot (.png)")), br(), br(),
    downloadButton(ns('dlBcPlot'), i18n$t("Download"))
  )
}
