...
selectOccs_MOD <- function(input, output, session, rvs) {
  ...
  reactive({
    if (is.null(rvs$occs)) {
      rvs %>% writeLog(type = 'error', "Before processing occurrences, obtain the data in component 1.")
      return()

    }
    if (is.null(rvs$polySelXY)) {
      rvs %>% writeLog(type = 'error', 'The polygon has not been finished. Please press "Finish" on the map toolbar, then the "Select Occurrences" button.')
      return()
