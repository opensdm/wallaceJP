...
mapPredsMaxent_UI <- function(id) {
  ns <- NS(id)
  tagList(
    tags$div(title=i18n$t("Please see guidance for an explanation of different Maxent output types."),
             radioButtons(ns('predType'), label = i18n$t("Prediction output"),
  ...
  reactive({
    ...
    if (input$predType == 'logistic') {
      ...
      if (is.null(rvs$modPredsLog)) {
        withProgress(message = i18n$t("Generating logistic predictions..."), {
      ...
      if (is.null(rvs$modPredsCLL)) {
        withProgress(message = i18n$t("Generating cloglog predictions..."), {
    ...
    if (rvs$clamp == T | rvs$algMaxent == "maxent.jar") {
      rvs %>% writeLog("Maxent", input$predType, i18n$t("clamped model prediction plotted."))
    } else if (rvs$clamp == F) {
      rvs %>% writeLog("Maxent", input$predType, i18n$t("unclamped model prediction plotted."))
    }
