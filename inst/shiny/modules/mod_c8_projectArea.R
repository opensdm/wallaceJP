...
projectArea_MOD <- function(input, output, session, rvs) {
  ...
  reactive({
    ...
    if (rvs$comp6 == 'bioclim') {
      rvs %>% writeLog(i18n$t('New area projection for BIOCLIM model with extent coordinates:'), coordsChar)
    } else if (rvs$comp6 == 'maxent') {
      if (rvs$clamp == T | rvs$algMaxent == "maxent.jar") {
        rvs %>% writeLog(i18n$t('New area projection for clamped model'), rvs$modSel, i18n$t('with extent coordinates:'), 
                         coordsChar)
      } else if (rvs$clamp == F) {
        rvs %>% writeLog(i18n$t('New area projection for unclamped'), rvs$modSel, i18n$t('with extent coordinates:'), 
                         coordsChar)
      }
    }
